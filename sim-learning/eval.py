...
IM_SIZE = 224
sim_model = FeatureExtractor(None, IM_SIZE)
...
def eval(args):
    ...
    # start = time.time()
    # attended_features = torch.zeros((80, len(val_data), 1024)).cuda()
    ...
    #         start = time.time()
    #
    # torch.save(attended_features, './features1.pth')

    # attended_features = torch.load('./features1.pth')
    # top_k = np.array([10, 30, 100, 300, 500, 1000, 2000], dtype=float)
    # tot_recall_sum = np.array([0, 0, 0, 0, 0, 0, 0], dtype=float)
    # tot_precision_sum = np.array([0, 0, 0, 0, 0, 0, 0], dtype=float)
    # tot_query = 0
    # for cls in range(16, 80):
    #     cls_recall_sum = np.array([0, 0, 0, 0, 0, 0, 0], dtype=float)
    #     cls_precision_sum = np.array([0, 0, 0, 0, 0, 0, 0], dtype=float)
    #
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trained_model', default='../../repo/sim/sim_1_3.pth', type=str)
    parser.add_argument('--attention', default=True)

    args = parser.parse_args()

    state_dict = torch.load(args.trained_model)
    sim_model.load_state_dict(state_dict)
    eval(args)
