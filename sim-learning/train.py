...
class TripletLoss(nn.Module):
    ...
    def forward(self, input, positive_pair, negative_pair):
        ...
        ranking_loss = torch.max(zero, alpha - 0.001 * pos_sim + neg_sim)
...
def train(epoch):
    ...
    for index, data in enumerate(train_loader):
        img, _, _, image_level_label, _, _, _, _, _ = data
        ...
        pos_img, _, _, _, _, _, _, _, _ = train_data[pos_sel]
        pos_img = pos_img.unsqueeze(0)
        pos_img = Variable(pos_img).cuda()
        neg_img, _, _, _, _, _, _, _, _ = train_data[neg_sel]
        ...
        if index % 10 == 0:
            ...
            cum_loss = 0
            cnt = 0
    torch.save(net.state_dict(), nowTime+':ep'+str(epoch)+'.pt')
