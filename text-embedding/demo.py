...
heat_map = np.zeros((20, IMG_SIZE // 16, IMG_SIZE // 16))

for cls in range(20):
    tot_cnt = dataset.cls_len(cls)
    query_img_raw, gt_boxes, h, w, im_id = dataset.pull_item(cls, 0)
    query_img_raw = cv2.resize(query_img_raw, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_LINEAR)
    ...
    for i in range(1, min(num_search + 1, dataset.cls_len(cls))):
        raw_im, gt_boxes, h, w, im_id = dataset.pull_item(cls, i)
        raw_im = cv2.resize(raw_im, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_LINEAR)
    ...
    print(avg_feat)
    for i, j in itertools.product(range(IMG_SIZE // 16), range(IMG_SIZE // 16)):
        ...
        heat_map[cls, i, j] = affinity.data[0]
    #print(heat_map)
...
for cls in range(20):
    scam = heat_map[cls, :, :]
    scam = scam - np.min(heat_map)
    scam = scam / np.max(heat_map)
    ...
    cv2.imwrite('./haha/globalcosine%d.jpg' % cls, sr)
